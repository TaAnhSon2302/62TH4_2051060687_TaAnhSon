...
public abstract class Popups : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        //     CameraController.Instance.isCameraMoveable = false;
        // if (TouchManager.Instance != null)
        //     TouchManager.Instance.IsAllowAdjustFurn = false;
    }

    private void OnDisable()
    {
        // if (CameraController.Instance != null)
        //     CameraController.Instance.isCameraMoveable = true;
        // if (TouchManager.Instance != null)
        //     TouchManager.Instance.IsAllowAdjustFurn = true;
    }
    ...
    public virtual void Appear()
    {
        ...
        if (Panel != null && _isAnimateAppear)
        {
            LeanTween.value(gameObject, 0, 1, 0.4f)
                .setOnUpdate((float value) => { Panel.transform.localScale = Vector3.one * value; }).setEaseOutBounce()
                .setOnComplete(() => { });
        }
        else
        {
        ...
        if (IsLoadBoxCollider)
        {
            if (_collider == null)
            {
                _collider = Instantiate(Resources.Load<GameObject>("Prefabs/Collider/Collider"), Panel.transform,
                    false);
            }
        }
    }
    ...
    public virtual void Disappear(Action onClose = null)
    {
        ...
        if (Panel != null && _isAnimateAppear)
        {
            LeanTween.value(gameObject, 1, 0, 0.2f)
                .setOnUpdate((float value) => { Panel.transform.localScale = Vector3.one * value; }).setEaseInSine()
                .setOnComplete(() =>
                {
                    onClose?.Invoke();
                    Root.alpha = 0;
